%% Version of the standard spm_MDP_VB_X.m SPM routine used for the vibrating capsule model

function [MDP] = spm_MDP_VB_X_Cap(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                  % number of MDPs
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_Cap(MDP(:,i),OPTIONS);
...
try, etaB   = MDP(1).etaB;   catch, etaB   = 1;    end

try, etaAV   = MDP(1).etaAV;   catch, etaAV   = 1;    end
try, etaANV   = MDP(1).etaANV;   catch, etaANV   = 1;    end

try, etaBV   = MDP(1).etaBV;   catch, etaBV   = 1;    end
try, etaBNV   = MDP(1).etaBNV;   catch, etaBNV   = 1;    end
...
% CH EDIT: Commented out for moving policies
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_Cap(mdp);
        ...
        if ~HMM || T == t
            ...
            if isfield(MDP,'U')
                %R = t;
                R = size(MDP(m).U, 1); % CH Edit: Changed here
            else
                R = S;
            end
            F     = zeros(Np(m),1);
    ...
    for t = 1:T
        ...
        if isfield(MDP,'a')
            for g = 1:Ng(m)
                ...
                % TODO:
                if t==T
                    if MDP(m).o(:, 2) == 2 && MDP.fit_options.a_mode == 2
                        MDP(m).a{g} = MDP(m).a{g} + da*etaANV;
                    elseif MDP(m).o(:, 2) == 3 && MDP.fit_options.a_mode == 2
                        MDP(m).a{g} = MDP(m).a{g} + da*etaAV;
                    else
                        MDP(m).a{g} = MDP(m).a{g} + da*etaA;
                    end
                end
                % Add split for etaNTA and etaTA (same as below)
                % change below to etaNTB and etaTB
                
                % Also change both eta's to etaA and etaB respectively
        ...
        if isfield(MDP,'b') && t > 1
            for f = 1:Nf(m)
                for k = 1:Np(m)
                    ...
                    if t==T
                        if MDP(m).o(:, 2) == 2 && MDP.fit_options.b_mode == 2
                            MDP(m).b{f}(:,:,v) = MDP(m).b{f}(:,:,v) + db*etaBNV;
                        elseif MDP(m).o(:, 2) == 3 && MDP.fit_options.b_mode == 2
                            MDP(m).b{f}(:,:,v) = MDP(m).b{f}(:,:,v) + db*etaBV;
                        else
                            MDP(m).b{f}(:,:,v) = MDP(m).b{f}(:,:,v) + db*etaB;
                        end
                    end
